...
pub fn ensure(paths: &TlsPaths, san_list: &[String]) -> Result<()> {
    ...
    leaf_params.distinguished_name = {
        ...
        dn.push(DnType::CommonName, "forge-web");
        dn
    };
    ...
    // Write leaf + CA as a full chain (leaf first, CA second). See the
    // twin comment in forge-server/src/tls_autogen.rs for rationale.
...
fn mint_ca(paths: &TlsPaths) -> Result<(String, KeyPair)> {
    ...
    params.distinguished_name = {
        let mut dn = DistinguishedName::new();
        dn.push(DnType::CommonName, "forge-web local CA");
        dn
    };
