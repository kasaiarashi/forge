...
export default function DocsPage() {
  return (
    ...
    >
      <p>
        Forge VCS is a binary-first version control system for Unreal
        Engine projects: BLAKE3 + FastCDC + zstd, file locking, gRPC
        transport, MIT-licensed and self-hosted. The pages below are
        enough to get you committing on your first day.
      </p>

      <Section title="Install the CLI">
        <p>
          Grab the latest <code>forge</code> binary from your operator (or
          the project releases page) and put it on your <code>PATH</code>.
          Verify with:
        </p>
      ...
      <Section title="First-time setup">
        <p>
          Log in to your Forge server. The CLI auto-detects whether the URL
          you give it is the web UI or the gRPC endpoint, prompts you to
          trust the server's TLS certificate on the first connect, then
          stores a long-lived personal access token in your OS keychain.
        </p>
...
forge login --server https://forge.example.com:3000`}</Code>
        <p>
          ...
          channel) before typing <code>y</code>. Subsequent commands use
          the pinned cert silently.
        </p>
      </Section>
...
forge push`}</Code>
      ...
      <Section title="Day-to-day commands">
        <Table
          rows={[
            ['forge status', 'Working tree status (modified, staged, untracked)'],
      ...
      <Section title="File locking (Unreal-friendly)">
        <p>
          Binary assets like <code>.uasset</code> and <code>.umap</code>{' '}
          can't be merged. Forge locks let you signal to teammates that
          you're about to edit one.
        </p>
...
forge unlock Content/Maps/MainLevel.umap`}</Code>
      ...
      <Section title="Personal access tokens">
        <p>
          The PAT minted by <code>forge login</code> has{' '}
          <code>repo:read</code> + <code>repo:write</code> scopes. For CI
          jobs, mint a narrower token from the web UI's settings page (or
          delete the auto-PAT and re-login with <code>--token</code>).
        </p>
...
forge-server repo list-members alice/game`}</Code>
      ...
      <Section title="Configuration files">
        <p>
          ...
          <code>forge-web.toml</code> (the browser-facing HTTPS frontend).
          Both auto-generate sensible defaults on first start. The most
          relevant knobs:
        </p>
        <ul>
          <li>
            <code>[server.tls].auto_generate = true</code> — mint a local
            CA + leaf cert on first run, reuse on every restart. Default
            on.
          </li>
          <li>
            <code>[actions].enabled</code> — opt in to the workflow engine.
            Default <strong>off</strong> because steps run as shell
          ...
          <li>
            <code>[storage.base_path]</code> — where everything lives on
            disk.
          </li>
        </ul>
      </Section>
      ...
      <Section title="More">
        <p>
          ...
          <code>README.md</code> for design notes and{' '}
          <code>CLAUDE.md</code> for the architectural overview.
        </p>
      </Section>
    </LegalLayout>
  );
