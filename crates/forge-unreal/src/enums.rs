//! Numeric version enums for `EUnrealEngineObjectUE4Version`,
//! `EUnrealEngineObjectUE5Version`, `EPackageFlags` and `EObjectFlags`.

use num_derive::FromPrimitive;
use std::fmt;

/// UE4 object-version discriminants. Values are the literal serialized integers
/// from `Engine/Source/Runtime/Core/Public/UObject/ObjectVersion.h`.
...
/// UE5 object-version discriminants from `EUnrealEngineObjectUE5Version`.
#[allow(non_camel_case_types, clippy::upper_case_acronyms)]
#[derive(Clone, Copy, Debug, FromPrimitive, PartialEq, PartialOrd)]
pub enum ObjectVersionUE5 {
    ...
    // 1017 is intentionally absent (no public name in the engine source).
...
/// `EObjectFlags` bit values (currently only re-exported for downstream consumers).
#[derive(Debug)]
#[repr(u32)]
